...
export interface BookRequestRow {
  ...
  due_on: string | null; requested_at: string; title: string; cover_url: string | null; book_price_minor: string | number; owner_user_id: number;
...
export interface BookRequestEventRow {
  action: string;
  note: string | null;
  actorName: string | null;
  at: string;
}
export interface AdminBookRequestRow extends BookRequestRow {
  events?: BookRequestEventRow[];
  canonical_key: string;
  owner_village?: string | null;
  owner_ward_no?: number | null;
  requester_village?: string | null;
  requester_ward_no?: number | null;
  requester_father_name?: string | null;
...
export function getAdminBookRequests(params: { search?: string; status?: number | ''; limit?: number; offset?: number } = {}) {
  return apiRequest<AdminBookRequestListResult>(`/books/admin/requests${createQueryString(params)}`);
}
