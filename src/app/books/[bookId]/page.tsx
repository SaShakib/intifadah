...
import { ArrowLeft, BookOpen, CalendarDays, Pencil, Send, Trash2, Users } from 'lucide-react';
...
export default function BookDetailPage() {
  ...
  const canDeleteBook = isOwner || canDeleteAnyBook;
  ...
  const remove = async () => {
    ...
    try {
      ...
      setToast(error instanceof Error ? error.message : 'বইটি সরানো যায়নি');
    } finally {
      setBusy(false);
    }
  };
  return <main className="min-h-screen bg-surface-2">
    ...
    <article className="mx-auto grid max-w-5xl gap-8 px-4 py-8 md:grid-cols-[300px_1fr]">
      ...
      <div><div className="flex flex-wrap gap-2">{(book.categories ?? []).length ? book.categories!.map((categoryRef) => (categoryRef.slug ? <Link key={categoryRef.id} href={`/books?category=${encodeURIComponent(categoryRef.slug)}`} className="rounded-full border border-brand/30 bg-brand-light px-3 py-1 text-xs font-semibold text-brand hover:bg-brand hover:text-white">{categoryRef.categoryName}</Link> : <span key={categoryRef.id} className="rounded-full border border-brand/30 bg-brand-light px-3 py-1 text-xs font-semibold text-brand">{categoryRef.categoryName}</span>)) : <span className="text-sm font-semibold text-brand">বিভাগহীন</span>}</div><h1 className="mt-3 text-3xl font-bold text-fg">{book.title}</h1><p className="mt-2 text-lg text-fg-2">{book.author_name || 'লেখক অজানা'}</p><dl className="mt-7 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-muted">মালিক</dt><dd className="mt-1 font-semibold text-fg">{book.owner_name}</dd></div><div><dt className="text-muted">হারালে মূল্য</dt><dd className="mt-1 font-semibold text-fg">৳{Number(book.book_price_minor)}</dd></div></dl><div className="mt-5 space-y-2 text-sm text-muted"><p className="flex items-center gap-2"><Users className="h-4 w-4 text-brand" />{totalCopies > 1 ? `${availableCopies}/${totalCopies} কপি এখন উপলব্ধ` : availableCopies ? 'কপি এখন উপলব্ধ' : 'কপি এখন ধার দেওয়া আছে'}</p>{estimatedFree && <p className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-brand" />সম্ভাব্য ফ্রি: {estimatedFree}</p>}</div>{book.description && <p className="mt-7 leading-7 text-fg-2">{book.description}</p>}<div className="mt-8 flex flex-wrap gap-2">{isAuthenticated && !isOwner ? <Button disabled={!availableCopies} onClick={() => setOpen(true)}><Send className="h-4 w-4" />{availableCopies ? 'ধার নেওয়ার অনুরোধ' : 'এখন কোনো কপি নেই'}</Button> : !isAuthenticated ? <Button onClick={() => router.push('/login?books=1')}><Send className="h-4 w-4" />লগইন করে অনুরোধ করুন</Button> : null}{isOwner && <Button variant="secondary" onClick={() => setEditOpen(true)}><Pencil className="h-4 w-4" />সম্পাদনা করুন</Button>}{canDeleteBook && <Button variant="danger" onClick={() => setDeleteOpen(true)}><Trash2 className="h-4 w-4" />সরিয়ে দিন</Button>}<ShareButton url={`/books/${book.id}`} title={book.title} text={`বই দেখুন: ${book.title}`} size="md" /></div></div>
    </article>
