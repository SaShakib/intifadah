  ...
  return (
    <main className="min-h-screen bg-surface-2 pb-[calc(var(--bottomnav-h)+1.5rem)] md:pb-12">
      ...
      <section className="mx-auto max-w-6xl space-y-5 px-4 py-7">
        ...
        {incomingRequests.length > 0 && <div className="border border-border bg-white p-5"><h3 className="font-bold text-fg">আমার বইয়ের জন্য আসা অনুরোধ</h3><div className="mt-4 space-y-3">{incomingRequests.map((request) => { const pendingExtension = request.extensions?.find((extension) => extension.status === 0); return <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3 text-sm last:border-0"><div><p className="font-semibold text-fg">{request.title}</p><p className="text-xs text-muted">অনুরোধকারী: {request.requester_name} · {request.requested_days} দিন{request.due_on ? ` · সম্ভাব্য ফেরত: ${bookDate(request.due_on)}` : ''}</p></div><div className="flex flex-wrap gap-2">{request.status === 0 && <><Button size="sm" disabled={busy} onClick={() => void updateRequest(request, 'accept')}>গ্রহণ</Button><Button size="sm" variant="danger" disabled={busy} onClick={() => void updateRequest(request, 'reject')}>প্রত্যাখ্যান</Button></>}{request.status === 1 && <Button size="sm" disabled={busy} onClick={() => void updateRequest(request, 'given')}>দিয়েছি</Button>}{request.status === 5 && <Button size="sm" disabled={busy} onClick={() => void updateRequest(request, 'return_received')}>ফেরত গ্রহণ নিশ্চিত</Button>}{request.status != 6 && pendingExtension && <><Button size="sm" disabled={busy} onClick={() => void updateExtension(pendingExtension.id, true)}>+{pendingExtension.requestedDays} দিন</Button><Button size="sm" variant="danger" disabled={busy} onClick={() => void updateExtension(pendingExtension.id, false)}>না</Button></>}</div></div>; })}</div></div>}
