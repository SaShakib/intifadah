  ...
  return (
    <main className="min-h-screen bg-surface-2 pb-12">
      ...
      {grouped ? (
          ...
          {sections.map((section) => (
            <section key={section.categoryId} className="mx-auto max-w-6xl px-4 pt-7">
              <div className="flex items-center justify-between gap-3">
                ...
                {section.slug && <Link href={`/books?category=${encodeURIComponent(section.slug)}`} className="text-sm font-semibold text-brand hover:underline">সব দেখুন</Link>}
              </div>
        ...
        <section className="mx-auto grid max-w-6xl gap-4 px-4 py-7 sm:grid-cols-2 lg:grid-cols-4">{books.map(renderBook)}</section>
      )}
