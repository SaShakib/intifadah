...
export default function TrackSection() {
  ...
  const progressRows: ApiQuranProgressRow[] = useMemo(
  ...
  const saveProgress = async (section: 'quran' | 'namaj') => {
    ...
    try {
      ...
      if (selectedRecord) {
        ...
        showToast(section === 'quran' ? 'Quran রেকর্ডের তথ্য আপডেট করা হয়েছে।' : 'Namaj রেকর্ডের তথ্য আপডেট করা হয়েছে।');
      } else {
        await createUserQuranProgress(input);
        showToast(selectedDate === today()
          ? `আজকের ${section === 'quran' ? 'Quran' : 'Namaj'} রেকর্ড করা হয়েছে।`
          : `${section === 'quran' ? 'Quran' : 'Namaj'} রেকর্ড সংরক্ষণ হয়েছে।`);
      }
      await refetch();
  ...
  const metrics = useMemo(() => {
    ...
    return [
      ...
      { label: 'আজ', value: todayDone ? 'পড়া হয়েছে' : 'বাকি', hint: todayDone ? 'আজকের রেকর্ড আছে' : 'এখনও রেকর্ড করা হয়নি' },
      { label: 'মোট পৃষ্ঠা', value: String(pages), hint: 'দেওয়া তথ্য থেকে' },
      { label: 'মোট সময়', value: `${minutes} মিনিট`, hint: 'দেওয়া তথ্য থেকে' },
    ];
    ...
    <>
      ...
      <Card className="border-brand/20 bg-brand-light/25">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {selectedDateDone && <span className="inline-flex items-center gap-1 rounded-full bg-success/10 px-3 py-1.5 text-xs font-bold text-success"><CheckCircle2 className="h-4 w-4" />রেকর্ড করা হয়েছে, তথ্য আপডেট করা যাবে</span>}
        </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <Card>
          <SectionHeader title="Quran রেকর্ড" subtitle="পড়া হলে তথ্য দিন, না দিলেও শুধু দিনটির Quran রেকর্ড সংরক্ষণ করা যাবে" />
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="space-y-1"><span className="text-xs font-semibold text-fg-2">সময়</span><Input type="number" value={form.minutesRead ?? ''} onChange={(event) => updateForm('minutesRead', event.target.value ? Number(event.target.value) : null)} placeholder="মিনিট (ঐচ্ছিক)" /></label>
        ...
        <Card>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="space-y-1"><span className="text-xs font-semibold text-fg-2">জামাতে পড়া হয়েছে কত ওয়াক্ত?</span><Input type="number" min="0" max="5" value={form.congregationalPrayers ?? ''} onChange={(event) => updateForm('congregationalPrayers', event.target.value ? Number(event.target.value) : null)} placeholder="০ থেকে ৫" /></label>
            <label className="space-y-1"><span className="text-xs font-semibold text-fg-2">নফল নামাজ পড়া হয়েছে কত রাকাত?</span><Input type="number" min="0" value={form.naflRakat ?? ''} onChange={(event) => updateForm('naflRakat', event.target.value ? Number(event.target.value) : null)} placeholder="রাকাত সংখ্যা" /></label>
            <div className="sm:col-span-2">
      ...
      <section>
        <Card>
          ...
          <DataTable
            ...
            searchPlaceholder="তারিখ দিয়ে খুঁজুন..."
      ...
      <section>
        <Card>
          ...
          {loading ? <ApiLoadingNotice label="Quran ও Namaj রেকর্ড লোড হচ্ছে..." /> : (
            <DataTable
              headers={['তারিখ', 'পৃষ্ঠা', 'সুরা', 'সময়', 'নোট']}
              rows={quranRows}
        ...
        <Card>
          ...
          {loading ? <ApiLoadingNotice label="Namaj রেকর্ড লোড হচ্ছে..." /> : (
            <DataTable
              ...
              rows={namajRows}
              searchPlaceholder="তারিখ বা নোট দিয়ে খুঁজুন..."
