...
export function BookEditModal({ book, open, onClose, onUpdated, onMessage }: BookEditModalProps) {
  ...
  return <AppModal open={open} title="বই সম্পাদনা করুন" onClose={onClose} className="max-w-3xl" loading={searchingCover || saving} loadingLabel={saving ? 'তথ্য সংরক্ষণ হচ্ছে...' : 'কভার খোঁজা হচ্ছে...'} footer={<><Button variant="secondary" onClick={onClose}>বাতিল</Button><Button disabled={saving || searchingCover} onClick={() => void save()}><Check className="h-4 w-4" />পরিবর্তন সংরক্ষণ করুন</Button></>}>
    <div className="space-y-4">
      ...
      <label onClick={() => { if (!coverSearchQuery) onMessage('প্রথমে বইয়ের কভার খুঁজুন। কভার না পেলে তারপর ছবি আপলোড করুন।'); }} className={`flex items-center gap-2 rounded-lg border border-dashed p-3 text-sm font-semibold ${coverSearchQuery ? 'cursor-pointer border-border' : 'cursor-not-allowed border-border bg-surface-2 text-muted'}`}>
        <ImagePlus className="h-4 w-4" />নতুন কভার আপলোড করুন <span className="font-normal text-muted">(Google-এ না পেলে)</span>
        <input className="hidden" type="file" accept="image/*" disabled={!coverSearchQuery} onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; setSaving(true); try { const upload = await uploadBookCover(file); setForm({ ...form, ...upload, externalSource: '', externalVolumeId: '' }); onMessage('WebP কভার আপলোড হয়েছে'); } catch (error) { onMessage(error instanceof Error ? error.message : 'কভার আপলোড হয়নি'); } finally { setSaving(false); } }} />
      </label>
      {!coverSearchQuery && <p className="-mt-2 text-xs leading-5 text-muted">আগে বইয়ের নাম দিয়ে খুঁজুন। ফলাফলে কভার না পেলে তখন নিজের ছবি আপলোড করতে পারবেন।</p>}
