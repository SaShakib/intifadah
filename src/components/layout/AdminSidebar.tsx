...
export function AdminSidebar({ isOpen, onClose }: AdminSidebarProps) {
  ...
  const sections = [...new Set(ADMIN_NAV_ITEMS.map((item) => item.section))];
      ...
      >
        <div className="border-b border-white/10 px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/20 text-lg font-bold">ই</div>
            <div>
        ...
        <div className="border-b border-white/10 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-white">{user?.name ?? 'আবু বক্কর সিদ্দিক'}</p>
              <p className="text-xs text-white/60">ম্যানেজার</p>
            </div>
          </div>
        </div>

        <nav className="h-[calc(100%-9rem)] overflow-y-auto px-2 py-2">
